...
-  For step-by-step guidance to create a test lab environment on Windows Azure that includes AD DS, see [Test Lab Guide: Windows Server 2012 R2 Base Configuration in Windows Azure](http://www.microsoft.com/en-us/download/details.aspx?id=41684).
...
To configure...  | On-premises  | Windows Azure virtual network	
------------- | -------------  | ------------
...
1. Sign in to the [Windows Azure Management Portal](https://manage.windowsazure.com).
...
1. The dynamic IP address that the VM is assigned by default is valid for the duration of the cloud service. But it will change if the VM is shut down. You can assign a static IP address by [running the Set-AzureStaticVNetIP Windows Azure PowerShell cmdlet](http://msdn.microsoft.com/en-us/library/windowsazure/dn630228.aspx) so the IP address will persist if you ever do need to shut down the VM. 
...
Create additional VMS to provision domain member computers. You can use the UI or Windows Azure PowerShell. If you use the UI, just follow the same steps that you used to create the first VM. Then join the VMs to the domain just as you would on-premises. If you use Windows Azure PowerShell, you can provision VMs and have them domain-joined when they first start, as in the following example.
	'
